...
pub fn scan_str_lit(lit: &Literal) -> TokenStream {
    let s = lit.to_string();
    if !s.starts_with('\"') {
        return quote!(::shelle::Arg::from(#lit));
    }
    let mut iter = s[1..s.len() - 1] // To trim outside ""
        .chars()
        .peekable();
    let mut output = quote!(::shelle::Arg::default());
    ...
    while let Some(ch) = iter.next() {
        if ch == '#' {
            ...
            if !var.is_empty() {
                ...
                output.extend(quote!(.push(&#var)));
            } else {
                output.extend(quote!(.push("#")));
            }
        } else {
...
impl Lexer {
    pub fn new(input: TokenStream) -> Self {
        Self {
            ...
            iter: TokenStreamPeekable { peekable: input.into_iter().peekable(), span: Span::call_site() },
        }
    }
    ...
    fn add_arg_with_token(&mut self, token: SepToken, token_span: Span) {
        ...
        if let Some((redirect, span)) = self.last_redirect.take() {
            ...
            self.args.push(ParseArg::RedirectFile(fd, quote!(#last_arg_str), append));
            if stdouterr {
    ...
    fn extend_last_arg(&mut self, stream: TokenStream) {
        if self.last_arg_str.is_empty() {
            self.last_arg_str = quote!(::shelle::Arg::default());
        }
        self.last_arg_str.extend(quote!(.push(#stream)));
    }
    ...
    fn scan_ampersand(&mut self) {
        if let Some(tt) = self.iter.peek_no_gap() {
            ...
            abort!(self.iter.span(), "wrong redirection format: no spacing permitted before '&'");
    ...
    fn scan_hash(&mut self) {
        ...
        if let Some(TokenTree::Ident(var)) = peek_no_gap {
            self.extend_last_arg(quote!(&#var));
    ...
    fn peek_no_gap(&mut self) -> Option<&TokenTree> {
        match self.peekable.peek() {
            ...
            Some(item) => {
                ...
                if new_start > cur_end { None } else { Some(item) }
            }
        }
    }
